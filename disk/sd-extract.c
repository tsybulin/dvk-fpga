...
file  - имя файла для записи извлекаемого образа, по умолчанию B#-DD#.DSK\n\n\
 Пример: %s /dev/sdc RK 2 1 - извлечение образа RK2: из банка 1\n\n",argv[0],argv[0]);
    ...
    if (strncmp(DD,"RL",2) == 0) {
        char* outbuf;    
        // преобразование буфера для RL-дисков
        outbuf = malloc(realsize * 512) ;
        if (outbuf == 0) {
            printf("- недостаточно памяти под буфер\n");
            return;
        }
        
        memset(outbuf, 0, realsize * 512) ;
        int pin = 0 ;
        int pout = 0 ;
        for (int inblk = 0; inblk < usize; inblk++) {
            for (int inbyte = 0; inbyte < 512; inbyte++) {
                    if (inbyte < 256) {
                        outbuf[pout++] = buf[pin] ;
                    }
                    pin++ ;
